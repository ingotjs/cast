...
import { pmNames, type PmName } from "../lib/pkg-config";
...
export function PmTabs({ active, onSwitch }: { active: PmName; onSwitch: (pm: PmName) => void }) {
  const handleKeyDown = useCallback(
    (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowRight") newIdx = (idx + 1) % pmNames.length;
      else if (e.key === "ArrowLeft") newIdx = (idx - 1 + pmNames.length) % pmNames.length;
      if (newIdx !== undefined) {
        e.preventDefault();
