...
**Tests:** Unit tests live in `tests/` at the package root (e.g., `packages/api/tests/`). Locales in `locales/` at the package or source root.
...
- **Keep functions and components short.** Extract helpers, sub-components, and constants. Avoid long inlined JSX, objects, or logic blocks — break them into well-named pieces. Clean, readable code is always better than compact code.
