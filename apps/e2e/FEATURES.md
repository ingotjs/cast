...
- **#4**: Email verification is checked by reading captured emails from `packages/email/_etc/.email-captures/` (the dev server writes emails to JSON files instead of sending via Resend).
...
| `getEmails`         | Reads captured emails for a recipient from `packages/email/_etc/.email-captures/`. |
